...
public class SeriesWebViewActivity extends Activity {
    ...
    private String seriesurl = ImageUtil.SeriesUrls[0];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_series_web_view);
        initwidget();
        initData();
        initListener();
        Intent intent = getIntent();
        clickedposition = intent.getIntExtra("position",0);
        videowebview.loadUrl(seriesurl);
    }

    @Override
    protected void onResume() {
        super.onResume();
        videowebview.loadUrl(ImageUtil.SeriesUrls[clickedposition]);
    }

    private void initData() {
        mDatas = new ArrayList<String>();
        for (int i =0; i < ImageUtil.SeriesUrls.length ; i++){
            String data = ImageUtil.SeriesUrls[i];
            mDatas.add(data);
        }
    }
...
//     mTestWebView.loadData("", "text/html; charset=UTF-8", null);
                SeriesWebViewActivity.this.finish();
                Log.i("testwebview", "===>>>2");
            }
        }
        return true;
...
//    setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        ...
        public void onReceivedTitle(WebView view, String title) {
            (SeriesWebViewActivity.this).setTitle(title);
        }
//   @Override
//  //当WebView进度改变时更新窗口进度
